# Rewards Settlement (Demo → Mainnet) (Draft)

이 문서는 데모 기간에 쌓인 보상(오프체인 원장)을 메인넷에서 **에폭(Epoch) 단위 Merkle claim**으로 정산하는 설계를 정의합니다.
...
## 2) 데모 기간(Option A) 데이터 원칙

데모에서 서버는 온체인 전송/민팅을 수행하지 않고, 다음 원칙으로 **오프체인 원장만** 적립합니다.
