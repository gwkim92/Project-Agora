...
export async function GET(req: NextRequest, ctx: { params: Promise<{ submissionId: string }> }) {
  try {
    ...
    const data = await serverGetJson(`/api/v1/submissions/${submissionId}/comments${qs}`);
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ submissionId: string }> }) {
  try {
    ...
    const data = await serverPostJson(`/api/v1/submissions/${submissionId}/comments`, body, { token });
    return NextResponse.json(data);
  } catch (e) {
    return NextResponse.json({ detail: e instanceof Error ? e.message : "Bad Request" }, { status: 400 });
  }
