...
import { serverGetJson, serverPutJson } from "@/app/api/_lib/serverApi";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await serverGetJson("/api/v1/profile", { token });
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const data = await serverPutJson("/api/v1/profile", body, { token });
    return NextResponse.json(data);
  } catch (e) {
    return NextResponse.json({ detail: e instanceof Error ? e.message : "Bad Request" }, { status: 400 });
  }
